...
$sql = "SELECT * FROM `users`";
 require 'mailler/autoload.php';
  ...
  $result=mysqli_query($conn, "select * from tbl_product where expiration_date='$d'");
  $vrow=mysqli_fetch_array($result);
  $id = $vrow['id'];

  mysqli_query($conn,"UPDATE tbl_product SET rstat = 'EXPIRED' WHERE id = '$id'");
  ...
  $subject = 'AQT MANAGEMENT | PRODUCT EXPIRATION | '.strtoupper($product).' | '.$status.'';


 
 
  mysqli_query($conn,"INSERT INTO auto_mail_history (email, status, date_time) VALUES ( '$email', '$status', '$d')");
...
<head>
  <meta charset="UTF-8">
  <title>AQT MANAGEMENT | PRODUCT EXPIRATION</title>
...
<div class="container">
  <header id="main-header">
    <div id="title"><h2><strong>AQT MANAGEMENT</strong> <span>PRODUCT EXPIRATION</span></h2></div>
    <div class="info">
    </div>
  </header>
  ...
  <main>
    ...
    <section id="section2">
      ...
      <article>
        <div>
          <header>
            <h2><span>1</span> PRODUCT '.$product.' | DATE CREATED '.$vrow['date_created'].'  | EXPIRED TODAY</h2>
          </header>
        </div>
      </article>
  ...
  <footer>
    ...
    <a href="#" id="settings">AQT MANAGEMENT</a>
